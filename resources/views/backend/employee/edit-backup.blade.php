                            ...
                            <div class="tab-content border border-top-0 p-3">
                                ...
                                <div class="tab-pane fade" id="office_information" role="tabpanel">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <a class="btn btn-primary float-right mt-4" data-toggle="tab" href="#education" role="tab" aria-selected="false">Next</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="tab-pane fade" id="education" role="tabpanel">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <a class="btn btn-primary float-right mt-4" data-toggle="tab" href="#experience" role="tab" aria-selected="false">Next</a>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var experienceTable = $('#experience-table').removeAttr('width').DataTable({
                ...
                sorting : true,
                ajax: {
                    url: "/api/experience-api/"+{{$_GET['id']}},
                    type: "GET",
                },
                columns: [{
                    data: 'DT_RowIndex',
        ...
        function delete_experience(id){
            Swal.fire({
                ...
                if (result.value) {
                    window.location.href = `/api/experience-api/destroy/`+id
                    let url = $(this).attr('href');
                }
            }); //alert ends
        }
        ...
        $.ajax({
            url: "/api/employee-api/show/"+{{$_GET['id']}},
            type: "GET",
            success: function(response) {
                ...
                // console.log(response.office_information.department_id);
        ...
        $('#educationFormBtn').click(function (e){
           e.preventDefault() ;
            ...
            if (!education_type.length > 0){
                $("#msg_education_type").text('Education type is required!');
            }else if (!passing_year > 0){
                $("#msg_passing_year").text('Passing year is required!');
            }
            else if(!institute_name > 0){
                $("#msg_institute_name").text('Passing year is required!');
            }else{


                $.ajax({
                    url: "/api/education-api/store/"+{{$_GET['id']}},
                    type: "POST",
                    ...
                    success: function(response) {
                        ...
                        $('#gpa').val('');
                        $('#institute_name').val('');
                        window.location = '/employee/edit?id=' + {{$_GET['id']}}
                    },
                });
            }
        ...
        $('#experienceFormBtn').click(function (e){
            ...
            if (!previous_workplace.length > 0){
                $("#msg_previous_workplace").text('Previous workplace is required!');
            }else if (!experience_designation.length > 0){
                $("#msg_experience_designation").text('Designation is required!');
            }
            else if(!start_date.length > 0){
                $("#msg_start_date").text('Start date is required!');
            }else if(!end_date.length > 0){
                $("#msg_end_date").text('End date is required!');
            }else{
                $.ajax({
                    url: "/api/experience-api/store/"+{{$_GET['id']}},
                    type: "POST",
                    data: experienceData,
                    success: function(response) {
        ...
        $("#employeeForm_btn").click(function(e){
            ...
            // }
                ...
                $.ajax({
                    url: "/api/employee-api/update/"+{{$_GET['id']}},
                    type: "POST",
                    // contentType: 'multipart/form-data',
                    data: formData,
                    dataType: 'JSON',
